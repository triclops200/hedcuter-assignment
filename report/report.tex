...
\subsection{Use a different base color}
Currently, the stippling method used only uses white as as the base
color for concentrating points. I changed it to use any arbitrary RGB
space color as a reference background and redefined the density
function as the euclidean distance between the pixel and background in
CIELab space (under a D65 illuminant). The maximum lab distance was
calculated using the lab-extent.lisp file, which is used to normalize
the distance between 0 and 1.

\subsection{Paralellized the move_sites function}
Currently, the move_sites function is single threaded. In order to
